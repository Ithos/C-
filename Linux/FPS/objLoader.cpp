...
int objLoader::loadObject(std::string filename,std::vector<collisionPlane>* planes){
	int materialId=-1;//Identificador del material de colision
	ismaterial=false;
    isnormals=false;
    istexture=false;
	...
	for(int i=0;i<coord.size();i++){
		if((*coord[i])[0]=='#'){//Si el primer simbolo de la linea es # la ignoramos ya que es un comentario
			...
			if(coll && planes!=NULL){
				...
				planes->push_back(collisionPlane(vector3d(normals[b-1]->x,normals[b-1]->y,normals[b-1]->z),vector3d(vertex[a-1]->x,vertex[a-1]->y,vertex[a-1]->z),vector3d(vertex[c-1]->x,vertex[c-1]->y,vertex[c-1]->z),vector3d(vertex[d-1]->x,vertex[d-1]->y,vertex[d-1]->z),vector3d(vertex[e-1]->x,vertex[e-1]->y,vertex[e-1]->z),materialId));
			}else{
		...
		if(strncmp(tmp,"collision",9)==0){//ademas de poder emplear el identificador collision para colisiones genéricas podemos emplear identificadores como collision1 para identificar distintos tipos específicos 
			coll=true;
			materialId=-1;
			int id=0;
			int k=9;
	...
	std::cout << filename << " " << vertex.size() << " " << normals.size() << " " << faces.size() << " " << materials.size() << std::endl;
	...
	glNewList(num,GL_COMPILE);//Creamos una nueva lista y de momento solo compilamos
		...
		for(int i=0;i<faces.size();i++){
			if(faces[i]->texcoor[0]==0)istexture=false;//Analizamos si hay textura en caso de que no todo el modelo tenga
			else istexture=true;
...
unsigned int objLoader::loadTexture(const char* filename, bool generate){
	...
	for(int i=0;i<textureNames.size();i++){
		if(textureNames[i]==filename)return texture[i];//Si ya hemos cargado la textura simplemente devolvemos el identificador
	}
	...
	textureNames.push_back(std::string(filename));

...
void DrawCube(float height){
	...
	glBegin(GL_QUADS);
		...
		//Cara frontal
		glTexCoord2f(1.0,1.0);//Tenemos que idicar que vertices de la imagen se corresponden con que vertices de la textura, mediante esta funcion podemos recortar la textura o conseguir q se repita
		...
		glTexCoord2f(0.0,0.0);//El vertice 0/0 se correspode con la esquina inferior izquierda
...
void init(){
	...
	glEnable(GL_TEXTURE_2D);//Esto es para usar texturas 2d
	tex=load_texture("elec.bmp");
}

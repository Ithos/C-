#define GL_GLEXT_PROTOTYPES //Esto es para poder reservar memoria en la tarjeta gráfica
#include <SDL/SDL.h>
...
float f[]={-15.0,15.0,-15.0,5.0,5.0,-5.0,5.0,-5.0,-5.0,-5.0,-5.0,-5.0};
...
void init(){
	...
	glGenBuffers(1,&vbo);//Esto nos servirá para generar un buffer en la tarjeta gráfica, de esta forma podremos almacenar memoria en la tarjeta en lugar de en la RAM
	glBindBuffer(GL_ARRAY_BUFFER,vbo);
	glBufferData(GL_ARRAY_BUFFER,sizeof(f),f,GL_STATIC_DRAW);//Almacenamos datos en el buffer
	//Estructura del último parámetro GL_a_b a= STREAM | STATIC | YNAMIC / b= READ | COPY | DRAW
	
	float rep[]={-5,5,-5};
	
	glBufferSubData(GL_ARRAY_BUFFER,0,3*sizeof(float),rep);//Esta función permite modificar los datos almacenados en el buffer, el 0 es el offset
	//mirar glMapBuffer
}
...
float a[]={0,2,-14,1,0,0,-2,-2,-14,0,1,0,2,-2,-14,0,0,1};
unsigned int ind[]={0,1,2};//Tambien pueden emplearse indices en buffers, mirar documentacion o cpluplusguy
...
void display(){
    ...
    glEnableClientState(GL_VERTEX_ARRAY);//Habilitamos los arrays de vertices
    glBindBuffer(GL_ARRAY_BUFFER,vbo);//Esats funciones cambian si se emplean shaders
    glVertexPointer(3,GL_FLOAT,0,0);//Indicamos el array y la forma de interpretarlo /en caso de no usar el buffer el ultimo parametro es f, si no este parámetro es el offset en el buffer
    glDrawArrays(GL_QUADS,0,4);
    glEnableClientState(GL_COLOR_ARRAY);//Habilitamos el array de colores
    glBindBuffer(GL_ARRAY_BUFFER,0);//Dejamos de usar el buffer
    glVertexPointer(3,GL_FLOAT,6*sizeof(float),a);
    glColorPointer(3,GL_FLOAT,6*sizeof(float),&a[3]);
    glDrawElements(GL_TRIANGLES,3,GL_UNSIGNED_INT,ind);//Esta función permite reutilizar vertices empleando el array de indices
...
int main(int argc,char** argv){
	...
	glDeleteBuffers(1,&vbo);//Borramos el buffer
	delete tex;
	return 0;
}

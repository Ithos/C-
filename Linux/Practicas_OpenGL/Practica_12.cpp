...
void display(){
	...
	glDisable(GL_LIGHTING);//Deshabilitamos la luz para poder usar colores, podriamos no habilitarlo, pero me supondria borrar lineas de codigo y tal
	glPushMatrix();//Guardamos la matriz que tengamos, en este caso la identidad
		glTranslatef(-0.3,0.0,-8.0);//Cada vez que lo llamemos multiplica la matriz actual por una de traslacion que indiquemos moviendo cosas en uno o varios ejes
		...
		glColor4f(0.0,0.0,1.0,1.0);//Los parametros son r,g,b y alpha, que es el factor de transparencia, 1.0 implica que el objeto es solido, 0.0 implica que es completamente transparente 
	...
	glPushMatrix();
		glTranslatef(0.3,0.0,-7.0);//Esto es un ejemplo para que se vea como dos objetos giran en distintas direcciones

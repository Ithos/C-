...
void main(){
	...
	vec3 ambient=mAmbient*lAmbient;//Componente ambiental de la luz
	
	vec3 surf2light=normalize(lightPos-position);//Vector de la superficie a la fuente de luz
	...
	vec3 diffuse= dContribution*(mDiffuse*lDiffuse);//Componente difusa de la luz
	
	vec3 surf2view = normalize(-position);//un vector normal de la superficie a la posición del punto de vista
	...
	vec3 specular=sContribution*lSpecular*mSpecular;//Componente especular de la luz

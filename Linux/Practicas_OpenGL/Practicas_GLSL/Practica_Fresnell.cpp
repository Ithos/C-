...
void init(){
    ...
    fresnellShader=new shader("fresnell.vs","fresnell.frag");
...
void display(){
	...
	fresnellShader->useShader();
	
	glUniform3f(glGetUniformLocation(fresnellShader->getProgramId(),"cameraPosition"),cam.getLocation().getX(),cam.getLocation().getY(),cam.getLocation().getZ());
	
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_CUBE_MAP,cubemap);
	glUniform1i(glGetUniformLocation(fresnellShader->getProgramId(),"cubemap"),0);
	
	
	
	pipeline.updateMatrices(fresnellShader->getProgramId());//Tenemos que actualizar las matrices siempre antes de dibujar
	scene->draw(fresnellShader->getProgramId());
	fresnellShader->delShader();

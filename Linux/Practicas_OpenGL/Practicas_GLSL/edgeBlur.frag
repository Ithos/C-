...
void main(){
	...
	float margin=0.3;
	vec3 tex=vec3(texture2D(texture,pos).rgb);
	
	if(isVertical==1){
		if(distance(tex,texture2D(texture,vec2(pos.x,pos.y-pixelSize.y)).rgb)>margin||distance(tex,texture2D(texture,vec2(pos.x,pos.y+pixelSize.y)).rgb)>margin||distance(tex,texture2D(texture,vec2(pos.x,pos.y+2.0*pixelSize.y)).rgb)>margin||distance(tex,texture2D(texture,vec2(pos.x,pos.y-2.0*pixelSize.y)).rgb)>margin){
			vec2 curPos=vec2(pos.x,pos.y-4.0*pixelSize.y);
			for(int i=0;i<9;i++){
				result+=texture2D(texture,curPos)*value[i];
				curPos.y+=pixelSize.y;
			}
		}else result=texture2D(texture,pos);
	}else{
		if(distance(tex,texture2D(texture,vec2(pos.x-pixelSize.x,pos.y)).rgb)>margin||distance(tex,texture2D(texture,vec2(pos.x+pixelSize.x,pos.y)).rgb)>margin||distance(tex,texture2D(texture,vec2(pos.x+2.0*pixelSize.x,pos.y)).rgb)>margin||distance(tex,texture2D(texture,vec2(pos.x-2.0*pixelSize.x,pos.y)).rgb)>margin){

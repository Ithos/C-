...
void init(){
    ...
    initShader("vertex3.vs","fragment3.frag");
    myImg=load_texture("texture.jpg",true);
    myImg2=load_texture("capsule0.jpg",true);
}
...
void display(){
	...
	glUniform3f(glGetUniformLocation(program,"color"),0.0,0.0,1.0);//Mediante esta función pasámos datos a los shaders, los datos no cambian de un frame a otro, solo cambian cuando se vuelve a realizar el linkado del programa, tampoco deberían cambiar de un vertice a otro
	int index=glGetAttribLocation(program,"attr");
	
	//glEnable(GL_TEXTURE_2D); //Esto no es necesario pq estamos usando shaders, en caso de incluirlo, si no modificamos el shader no hace nada
	...
	glActiveTexture(GL_TEXTURE1);//Indicamos que la siguiente textura activa tendrá el indicador 1
	...
	glUniform1i(glGetUniformLocation(program,"img2"),1);//Con esto indicamos la textura que queremos cargar

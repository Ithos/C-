...
void init(){
    ...
    initShader("vertex5.vs","fragment5.frag");
    myImg=load_texture("texture.jpg",true);
    myImg2=load_texture("textureNormalmap.jpg",true);
}

void display(){
	
	if(b){//Esto es para desplazar la luz de izquierda a derecha
		lPos+=0.1;
		if(lPos>10.0)b=!b;
	}else{
		lPos-=0.1;
	...
	glUniform3f(glGetUniformLocation(program,"lightPos"),lPos,0.0,0.0);//Damos al shader propiedades y posiciones de la luz y del material
	...
	glUniform1i(glGetUniformLocation(program,"normMap"),1);//en la segunda textura pasamos el normalmap
	
    glBegin(GL_QUADS);
		glNormal3f(0.0,0.0,1.0);
		glTexCoord2f(0,0); //Si se quieren emplear varias texturas directamente con openGl se puede emplear glMultiTexCoord2f              
        glVertex3f(-1,1,-4);    
        glTexCoord2f(0,1);
        glVertex3f(-1,-1,-4);
        glTexCoord2f(1,1);

...
class bullet:baseclass{
	...
	public:
	bullet(SDL_Surface *img,int x,int y,int xvel,int yvel);//No incluimos destructor porque liberamos image cada vez que la usamos// 
	void show(SDL_Surface *screen); 
	void move(int playerXvel,int playerYvel);
	SDL_Rect* getRect();

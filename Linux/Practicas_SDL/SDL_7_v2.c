	...
	while(run){
		...
		while(SDL_PollEvent(&event)){//manejamos el evento; si no ocurre evento SDL_PollEvent=0 si ocurre !=0
			...
			if(event.type==SDL_KEYDOWN){
				...
				if(event.key.keysym.sym==SDLK_w)key_anim[0]=1;
				if(event.key.keysym.sym==SDLK_z)key_anim[1]=1;
				if(event.key.keysym.sym==SDLK_d)key_anim[2]=1;
				if(event.key.keysym.sym==SDLK_a)key_anim[3]=1;
			}
			if(event.type==SDL_KEYUP){
				...
				if(event.key.keysym.sym==SDLK_w)key_anim[0]=0;
				if(event.key.keysym.sym==SDLK_z)key_anim[1]=0;
				if(event.key.keysym.sym==SDLK_d)key_anim[2]=0;
				if(event.key.keysym.sym==SDLK_a)key_anim[3]=0;
			}
		...
		if(key_anim[0]&&rect1.y!=0)rect.y--;
		if(key_anim[1]&&rect1.y!=460)rect.y++;
		if(key_anim[2]&&rect1.x!=620)rect.x++;
		if(key_anim[3]&&rect1.x!=0)rect.x--;

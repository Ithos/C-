...
GeometryEngine::Spotlight::Spotlight(float maxLightAngle, const QVector3D & attParams, const QVector3D & direction, const QVector3D & diffuse,
	const QVector3D & ambient, const QVector3D & specular, const QVector3D & pos, const QVector3D & rot,
	const QVector3D & scale, WorldItem * parent) : Light(diffuse, ambient, specular, pos, rot, scale, parent), mAttenuationParameters(attParams), mDirection(direction), 
...
{
	mVertexShaderKey = LightShaderConstants::FLASHLIGHT_VERTEX_SHADER;
	mFragmentShaderKey = LightShaderConstants::FLASHLIGHT_FRAGMENT_SHADER;
}

void GeometryEngine::Spotlight::setProgramParameters(const LightingTransformationData & transformData, const MaterialLightingParameters & matParam, const QVector3D & viewPos)
{
	if (mpProgram != nullptr)
	{
		...
		mpProgram->setUniformValue("light.maxAngle", this->mMaxLightAngle);
...
void GeometryEngine::Spotlight::calculateContribution(QOpenGLBuffer * arrayBuf, QOpenGLBuffer * indexBuf, unsigned int totalVertexNum)
{
